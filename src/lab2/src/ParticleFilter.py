...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    # YOUR CODE HERE
    permissible_x, permissible_y = np.where(self.permissible_region == 1) # Check areas in the
    index = np.random.randint(0,len(permissible_x),self.N_PARTICLES)  # map which are in-bounds
    map_samples = np.zeros((self.N_PARTICLES,3)) # Initialize the map array 
    map_samples [:,0] = permissible_y[index]
    map_samples [:,1] = permissible_x[index]
    map_samples [:,2] = np.random.random(self.N_PARTICLES)*2.0*np.pi
    ...
    self.particles = map_samples  # Able to use the map_samples to update particles in place
    ...
    self.weights[:] = np.ones(self.N_PARTICLES) / float(self.N_PARTICLES)
                                          # dividing by the N number of particles
                                          #this is the changing the weights in-place
  ...
  def expected_pose(self):
    ...
    # Originally thought we were neeeding to calculate out all 
  '''
  ...
  def clicked_pose_cb(self, msg):
    ...
    # Here we thought we may need to check to see if there was an actual instance of the
    # message, but as it is a callback there is not need to do this check before 
    # calculating our new self.particles[] from the initial pose
    self.weights[:] = np.ones(self.N_PARTICLES)[:] / float(self.N_PARTICLES)
    self.particles[:,0] = msg.pose.pose.position.x + np.random.normal(0.0,0.5,self.N_PARTICLES)
    self.particles[:,1] = msg.pose.pose.position.y + np.random.normal(0.0,0.5,self.N_PARTICLES)
    self.particles[:,2] = Utils.quaternion_to_angle(msg.pose.pose.orientation) + np.random.normal(0.0,0.5,self.N_PARTICLES)
    self.state_lock.release()
