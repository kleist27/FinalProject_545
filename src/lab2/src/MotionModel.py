...
# Noise model values for velocity, steering angle, position, and direction
# Values are used in Gaussian distributions
KM_V_NOISE = 0.010 # Kinematic car velocity noise std dev
...
class KinematicMotionModel:
  ...
  def __init__(self, motor_state_topic, servo_state_topic, speed_to_erpm_offset, 
    ...
    self.local_deltas = np.zeros((1000, 3))
  '''
  ...
  '''
    ...
    particles
    Noise is added to kinematic car model equations to simulate real-world effects
  ...
  def motion_cb(self, msg):
    ...
    # Calculate current speed, v, from vesc message
    current_speed = (msg.state.speed - self.SPEED_TO_ERPM_OFFSET)/ self.SPEED_TO_ERPM_GAIN

    # Calculate current steering angle, delta, from vesc message
    current_steering_angle = (self.last_servo_cmd - self.STEERING_TO_SERVO_OFFSET) / self.STEERING_TO_SERVO_GAIN

    # change in time, dt, from message timestamp and last message timestamp
    dt = (msg.header.stamp.to_sec() - self.last_vesc_stamp.to_sec())

    # Gaussian distribution of noise around the current speed 
    speed_samples = np.random.normal(current_speed, KM_V_NOISE, self.particles.shape[0]) #1.00

    # Gaussian distribution of noise around the current steering angle
    steering_samples = np.random.normal(current_steering_angle, KM_DELTA_NOISE, self.particles.shape[0]) #0.34

    # Gaussian distribution of noise around the previous x position in particles array
    self.particles[:,0] = np.random.normal(self.particles[:,0], KM_X_FIX_NOISE, np.shape(self.particles[:,0]))

    # Gaussian distribution of noise around the previous y position in particles array
    self.particles[:,1] = np.random.normal(self.particles[:,1], KM_Y_FIX_NOISE, np.shape(self.particles[:,1]))

    # Gaussian distribution of noise around the previous theta in particles array
    self.particles[:,2] =  np.random.normal(self.particles[:,2], KM_THETA_FIX_NOISE, np.shape(self.particles[:,2]))

    # sin(2*Beta) value relative to the steering angle
    sin_2beta = np.sin(2*(np.arctan((.5 * np.tan(steering_samples)))))

    # store the previous theta values, theta_t-1
    prev_theta_t = np.zeros(self.particles.shape[0],dtype=float)
    prev_theta_t[:] = self.particles[:,2] 

    # calculate and store the current theta values based on the previous theta values in the kinematic equation
    self.particles[:,2] = (((speed_samples / self.CAR_LENGTH) * sin_2beta)*dt) + prev_theta_t
    theta_t = np.zeros(self.particles.shape[0],dtype=float)
    theta_t[:] = self.particles[:,2]

    # Recalculate the current x position based on the just calculated current theta values
    self.particles[:,0] += ((self.CAR_LENGTH/sin_2beta)*((np.sin(theta_t)) - (np.sin(prev_theta_t))))

    # Recalculate the current y position based on the just calculated current theta values
    self.particles[:,1] += ((self.CAR_LENGTH/sin_2beta)*((np.cos(prev_theta_t)) -(np.cos(theta_t)))) 
                      
    # Particles array now updated with current values with added noise models
...
TEST_DT = 1.0 #current_time_stamp.to_sec() - last_time_stamp.to_sec() #1.0 # seconds
